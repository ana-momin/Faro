...
function serviceIntentAssessment(body: string, includeTerms: string[], goal?: string, aiLabel?: string, aiConfidence = 0) {
  ...
  const requestHasLocalScope = BUYER_REQUEST_WITH_LOCAL_SCOPE_PATTERN.test(body) || DIRECT_AI_SERVICE_SCOPE_PATTERN.test(body);
