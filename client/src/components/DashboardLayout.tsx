...
import { Bookmark, Compass, LogOut, PlugZap, Radar, Search, Settings2, ShieldCheck, SlidersHorizontal, UserRound } from "lucide-react";
...
function DashboardLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  return <><Sidebar collapsible="icon" className="border-r border-[#eadfd2] bg-[#fffdfa]"><SidebarHeader className="h-[112px] justify-center"><div className="flex w-full justify-center px-3"><button onClick={() => setLocation(PRODUCT_INTRO_PATH)} className="rounded-2xl outline-none transition hover:scale-[1.02] active:scale-[0.97] focus-visible:ring-2 focus-visible:ring-[#bd674c]" title="About Faro AI" aria-label="Open Faro AI product introduction"><FaroLogo compact={state === "collapsed"} className="[&>span:first-child]:h-11 [&>span:first-child]:w-11 [&>span:first-child]:rounded-[14px] [&>span:last-child]:text-[24px]" /></button></div></SidebarHeader><SidebarContent className="px-2"><SidebarMenu className="mt-2">{menuItems.map(item => <SidebarMenuItem key={item.path}><SidebarMenuButton isActive={location === item.path} onClick={() => setLocation(item.path)} tooltip={item.label} className="h-10 rounded-xl text-[#4b3b2f] data-[active=true]:bg-[#f1d7b9] data-[active=true]:text-[#75432e] data-[active=true]:shadow-[inset_0_0_0_1px_rgba(185,105,77,0.16)]"><item.icon className="h-4 w-4" /><span>{item.label}</span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarContent><SidebarFooter className="p-3"><div className="mb-2 px-1 group-data-[collapsible=icon]:flex group-data-[collapsible=icon]:justify-center"><SidebarTrigger className="h-8 w-full rounded-xl border border-[#eadfd2] bg-white text-[#85533d] hover:bg-[#fff4e8] group-data-[collapsible=icon]:w-8" aria-label={state === "collapsed" ? "Expand sidebar" : "Minimize sidebar"} title={state === "collapsed" ? "Expand sidebar" : "Minimize sidebar"} /></div><DropdownMenu><DropdownMenuTrigger asChild><button className="flex w-full items-center gap-2.5 rounded-xl px-1 py-1.5 text-left hover:bg-[#fbf2e5] group-data-[collapsible=icon]:justify-center" aria-label="Open workspace settings" title="Workspace settings"><Avatar className="h-8 w-8 shrink-0"><AvatarImage src={user?.avatarUrl || undefined} alt={user?.name || "Faro member"} /><AvatarFallback className="bg-[#f8e4c8] text-[11px] font-semibold text-[#9c573f]">{user?.name?.charAt(0).toUpperCase() || "F"}</AvatarFallback></Avatar><div className="min-w-0 flex-1 group-data-[collapsible=icon]:hidden"><p className="truncate text-xs font-semibold">{user?.name || "Faro member"}</p><p className="truncate text-[10px] text-muted-foreground">{user?.email || "Private account"}</p></div><Settings2 className="h-3.5 w-3.5 shrink-0 text-[#a07862] group-data-[collapsible=icon]:hidden" aria-hidden="true" /></button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => setLocation("/saved")}><Bookmark className="mr-2 h-4 w-4" />Saved</DropdownMenuItem><DropdownMenuItem onClick={() => setLocation("/monitors")}><SlidersHorizontal className="mr-2 h-4 w-4" />Monitors</DropdownMenuItem><DropdownMenuItem onClick={() => setLocation("/provider")}><PlugZap className="mr-2 h-4 w-4" />Provider</DropdownMenuItem><DropdownMenuItem onClick={logout} className="text-destructive"><LogOut className="mr-2 h-4 w-4" />Sign out</DropdownMenuItem></DropdownMenuContent></DropdownMenu></SidebarFooter></Sidebar><SidebarInset>{isMobile && <div className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-[#eadfd2] bg-[#fffdfa]/95 px-3 backdrop-blur"><div className="flex items-center gap-2"><SidebarTrigger className="h-9 w-9" /><button onClick={() => setLocation(PRODUCT_INTRO_PATH)} className="rounded-lg outline-none focus-visible:ring-2 focus-visible:ring-[#bd674c]" aria-label="Open Faro AI product introduction"><FaroLogo /></button></div><span className="text-xs font-semibold text-[#856d5b]">{activeLabel}</span></div>}<main className="flex-1 p-3 sm:p-5 lg:p-7">{children}</main></SidebarInset></>;
}
