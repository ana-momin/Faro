...
function DashboardLayoutContent({ children, setSidebarWidth }: { children: React.ReactNode; setSidebarWidth: (width: number) => void }) {
  ...
  return <><div className="relative" ref={sidebarRef}><Sidebar collapsible="icon" className="border-r border-[#ececeb] bg-[#fbfbfa]" disableTransition={isResizing}><SidebarHeader className="h-[78px] justify-center"><div className="flex w-full justify-center px-3"><FaroLogo compact={isCollapsed} /></div></SidebarHeader><SidebarContent className="px-2"><div className="mx-1 mt-3 rounded-2xl bg-[#f0f2ee] p-2 group-data-[collapsible=icon]:hidden"><div className="flex items-center gap-2 px-2 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-[#7f827b]"><Radar className="h-3.5 w-3.5" />Faro desk</div></div><SidebarMenu className="mt-3">{menuItems.map(item => <SidebarMenuItem key={item.path}><SidebarMenuButton isActive={location === item.path} onClick={() => setLocation(item.path)} tooltip={item.label} className="h-10 rounded-xl text-[#30302f] data-[active=true]:bg-[#111214] data-[active=true]:text-white group-data-[collapsible=icon]:mx-auto"><item.icon className="h-4 w-4" /><span>{item.label}</span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarContent><SidebarFooter className="p-3"><div className="mb-3 flex items-center gap-2 rounded-xl bg-[#edf5ef] px-3 py-2 text-[10px] font-semibold text-[#467053] group-data-[collapsible=icon]:justify-center"><ShieldCheck className="h-3.5 w-3.5" /><span className="group-data-[collapsible=icon]:hidden">Human-led review</span></div><DropdownMenu><DropdownMenuTrigger asChild><button className="flex w-full items-center gap-2.5 rounded-xl px-1 py-1.5 text-left hover:bg-[#f1f1ef] group-data-[collapsible=icon]:justify-center"><Avatar className="h-8 w-8 shrink-0"><AvatarFallback className="bg-[#dff5e6] text-[11px] font-semibold text-[#17643c]">{user?.name?.charAt(0).toUpperCase() || "F"}</AvatarFallback></Avatar><div className="min-w-0 group-data-[collapsible=icon]:hidden"><p className="truncate text-xs font-semibold">{user?.name || "Faro member"}</p><p className="truncate text-[10px] text-muted-foreground">{user?.email || "Private account"}</p></div></button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => setLocation("/profile")}><UserRound className="mr-2 h-4 w-4" />Profile</DropdownMenuItem><DropdownMenuItem onClick={logout} className="text-destructive"><LogOut className="mr-2 h-4 w-4" />Sign out</DropdownMenuItem></DropdownMenuContent></DropdownMenu><button onClick={toggleSidebar} className="mt-3 flex h-8 w-full items-center justify-center rounded-lg text-[#777a73] transition hover:bg-[#f0f2ee]" title={isCollapsed ? "Expand sidebar" : "Collapse sidebar"} aria-label={isCollapsed ? "Expand sidebar" : "Collapse sidebar"}><PanelLeft className="h-4 w-4" /></button></SidebarFooter></Sidebar>{!isCollapsed && <div className="absolute right-0 top-0 z-50 h-full w-1 cursor-col-resize" onMouseDown={() => setIsResizing(true)} />}</div><SidebarInset>{isMobile && <div className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-[#ececeb] bg-[#fbfbfa]/95 px-3 backdrop-blur"><div className="flex items-center gap-2"><SidebarTrigger className="h-9 w-9" /><FaroLogo /></div><span className="text-xs font-semibold text-[#777773]">{activeLabel}</span></div>}<main className="flex-1 p-3 sm:p-5 lg:p-7">{children}</main></SidebarInset></>;
}
