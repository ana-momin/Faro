...
export function ProviderSetup({ setup, loading }: { setup: any; loading: boolean }) {
  ...
  return <section className="mt-5 max-w-3xl"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-[#a25d47]">Client-owned connection</p><h2 className="mt-1 text-2xl font-extrabold tracking-[-0.05em] text-[#432b1e]">Provider setup</h2><p className="mt-2 max-w-xl text-[11px] leading-5 text-[#8e7463]">Connect the account that pays for your source requests. Faro stores it encrypted and never displays the full key again.</p></div>{selectedProviderConnected ? <span className="inline-flex items-center gap-1.5 rounded-full border border-[#d1e4d5] bg-[#f4fbf5] px-3 py-2 text-[10px] font-extrabold text-[#3f7757]"><ShieldCheck className="h-3.5 w-3.5" />{selected.name} connected</span> : <span className="rounded-full border border-[#eed8bd] bg-[#fff8ed] px-3 py-2 text-[10px] font-extrabold text-[#9a613f]">Setup needed</span>}</div>
    ...
    <div className="mt-4 rounded-[26px] border border-[#eadfd2] bg-white p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-extrabold text-[#56392a]">{selected.name} credential</p><p className="mt-1 text-[10px] leading-5 text-[#977a68]">Each new Feed batch or Search run uses at most <strong className="text-[#754a36]">one provider request</strong>. Opening saved results uses no provider request.</p></div><div className="flex gap-2"><a href={selected.pricing} target="_blank" rel="noreferrer" className="inline-flex h-8 items-center gap-1 rounded-lg border border-[#eadfd2] px-2.5 text-[10px] font-extrabold text-[#83523c] hover:bg-[#fff4e8]">Pricing <ExternalLink className="h-3 w-3" /></a><a href={selected.setup} target="_blank" rel="noreferrer" className="inline-flex h-8 items-center gap-1 rounded-lg bg-[#f7e6d7] px-2.5 text-[10px] font-extrabold text-[#8f4e38] hover:bg-[#f2dac7]">Get key <ExternalLink className="h-3 w-3" /></a></div></div>
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><p className="text-[10px] leading-5 text-[#9a7a67]">{selectedProviderConnected ? <>Today: <strong className="text-[#704936]">{setup?.callsToday ?? 0}/{setup?.dailyRequestLimit ?? dailyRequestLimit}</strong> requests used.</> : "Set a daily limit before connecting this provider."} Background collection stays off by default.</p><button type="button" disabled={credential.trim().length < 12 || save.isPending} onClick={() => save.mutate({ provider, credential: credential.trim(), dailyRequestLimit })} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#b85f45] px-4 text-xs font-extrabold text-white shadow-[0_8px_18px_rgba(157,76,53,0.2)] hover:bg-[#9f4d36] disabled:cursor-not-allowed disabled:opacity-40"><KeyRound className="h-3.5 w-3.5" />{save.isPending ? "Saving…" : selectedProviderConnected ? "Update provider" : `Connect ${selected.name}`}</button></div>
    </div></section>;
}
