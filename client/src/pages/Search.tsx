...
import { ArrowRight, BadgeCheck, Bot, ChevronDown, Clapperboard, Code2, FlaskConical, History, Loader2, Megaphone, Radar, Send, WandSparkles, Workflow, type LucideIcon } from "lucide-react";
...
export default function Search() {
  ...
  return <div className="flex w-full items-start gap-6 pb-12">
    <SearchHistoryRail rows={historyRows} selectedMonitorId={activeResultMonitorId} onOpen={openHistory} />
    <div className="mx-auto w-full min-w-0 max-w-[1040px] flex-1">
      <header className="flex items-center justify-between border-b border-[#eadfd2] pb-5"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-[#a25d47]">Faro AI</p><h1 className="mt-0.5 text-lg font-extrabold tracking-[-0.05em] text-[#3d2e23]">Search</h1></div><div className="flex items-center gap-2"><MobileSearchHistory rows={historyRows} selectedMonitorId={activeResultMonitorId} onOpen={openHistory} /><span className="rounded-full border border-[#ead9c4] bg-white px-3 py-1.5 text-[10px] font-bold text-[#94624a]">Buyer-side only</span></div></header>
