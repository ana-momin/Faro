...
export default function Home() {
  ...
  return <DashboardLayout><div className="relative mx-auto max-w-[1550px] overflow-hidden rounded-[28px] bg-[#f8fafb] px-4 py-5 md:px-8 md:py-8">
    ...
    {showCreator && <section className="relative z-10 mt-6 rounded-[24px] border border-[#bed3e2] bg-white/85 p-5 shadow-[0_16px_50px_rgba(54,85,110,0.08)] backdrop-blur md:p-6">
      <div className="mb-5 flex items-start justify-between gap-6"><div><p className="text-xs font-medium uppercase tracking-[0.15em] text-[#6f8595]">Monitor builder</p><h2 className="mt-1 text-2xl font-bold tracking-[-0.05em]">Describe the listening goal.</h2><p className="mt-2 text-sm font-light text-[#647683]">Use plain English for a model-assisted suggestion, then review the X rule before saving.</p></div><button className="rounded-full p-2 hover:bg-[#edf3f7]" onClick={() => setShowCreator(false)} aria-label="Close monitor builder"><X className="h-4 w-4" /></button></div>
      <form onSubmit={createMonitor} className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="flex items-end gap-2"><label className="grid flex-1 gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-[#667b8b]">Plain-English goal<Textarea value={goal} onChange={event => setGoal(event.target.value)} className="min-h-20 rounded-xl border-[#d7e3ea] bg-white text-sm normal-case tracking-normal" /></label><Button type="button" variant="outline" className="mb-0 h-10 rounded-xl border-[#c9d9e4]" onClick={() => suggest.mutate({ goal })} disabled={suggest.isPending}><Sparkles className="mr-2 h-4 w-4" /> {suggest.isPending ? "Thinking" : "Suggest"}</Button></div>
        <label className="grid gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-[#667b8b]">Include terms<Input value={includeTerms} onChange={event => setIncludeTerms(event.target.value)} className="h-11 rounded-xl border-[#d7e3ea] bg-white text-sm normal-case tracking-normal" /></label>
    ...
    <section className="relative z-10 mt-6 grid gap-4 xl:grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr]">
      ...
      <Metric label="Live source" value={monitors.some(({ sync }) => sync?.source === "filtered_stream") ? "Stream" : "Search"} detail={monitors.find(({ sync }) => sync)?.sync?.latencyLabel ?? "Create or load a monitor"} />
    </section>
