...
const CONCRETE_BUYER_ACTION = "(looking for (?:someone|a freelancer|an? agency|a developer|an? engineer|a tester|a product tester|a designer|a creator|an? editor|an? expert|an? consultant)|looking to hire|need(?:s)? (?:someone|a freelancer|an? agency|a developer|an? engineer|a tester|a product tester|a designer|a creator|an? editor|an? expert|an? consultant|a team)|need(?:s)? help (?:building|automating|implementing|setting up|creating|integrating)|seeking (?:a provider|an? expert)|can someone (?:build|set up|implement)|who can (?:build|help us)|recommend (?:someone|a freelancer|an? agency)|looking to outsource|hire (?:a |an ))";
...
const OPENING_BUYER_REQUEST = /^\s*(?:looking for (?:someone|a freelancer|an? agency|a developer|an? engineer|a tester|a product tester|a designer|a creator|an? editor|an? expert|an? consultant)|looking to hire\b|need(?:s)? (?:someone|a freelancer|an? agency|a developer|an? engineer|a tester|a product tester|a designer|a creator|an? editor|an? expert|an? consultant|a team)|need(?:s)? help\b|seeking (?:a provider|an? expert)|can someone (?:build|set up|implement)\b|who can (?:build|help us)\b|recommend (?:someone|a freelancer|an? agency)|looking to outsource\b|hire (?:a |an ))/i;
...
export function getAllQualifiedPosts<T extends FeedItem>(items: T[]) {
  return items.filter(({ post }) => post.source !== "demo" && post.ruleScore >= 55 && isConcreteBuyerRequest(post));
}
